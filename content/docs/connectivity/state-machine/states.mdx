...
- **User can intervene** - Scan QR → BLE connection → WiFi provisioning at any time
...
5. If found and connected → silently transition to CONNECTING
6. If all fail → stay in RECOVERING, BLE visible for user
...
1. User scans QR code → App opens
2. App searches for BLE device (finds it because BLE is ON)
3. App verifies account ownership (security gate)
4. App shows available WiFi networks
5. User selects network → App sends credentials via BLE
6. Device receives credentials → Transition to CONNECTING
...
- Self-healing found a cached network → CONNECTING
- User provisioned new WiFi via BLE → CONNECTING
...
| **Very Weak** (< -80 dBm) | Give up after N retries | → RECOVERING |
...
- **If timeout/error:** WiFi works but no internet → Transition back to RECOVERING
...
**Success path:**
- WiFi connects AND verification passes → ONLINE (BLE turns OFF)

**Failure paths:**
- Connection fails (wrong password, AP unreachable) → RECOVERING
- Connection timeout (taking too long) → RECOVERING
- WiFi connected but verification fails (no internet) → RECOVERING
- Signal too weak after N retries → RECOVERING
...
### Device Display
- **"Dashboard showing IP address"** → ONLINE
- **"No Internet - Scan QR"** → RECOVERING
- **"Connecting to [Network]..."** → CONNECTING

### Mobile App
- **Green status, heartbeat working** → ONLINE
- **Red status, "Action Required"** → RECOVERING
- **Yellow status, "Connecting..."** → CONNECTING

### BLE Radio
- **Not discoverable/connectable** → ONLINE
- **Discoverable, broadcasting** → RECOVERING or CONNECTING

### What's Running
- **Only WiFi monitoring** → ONLINE
- **Self-healing + BLE provisioning** → RECOVERING
- **WiFi connection attempt + BLE safety net** → CONNECTING
