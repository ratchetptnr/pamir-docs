...
BLE turns ON
↓
User scans QR → BLE connects → App verifies account
↓
...
<Accordion title="Access Denied (Account Mismatch)">

**Scenario:** User connects via BLE but is not the registered owner of the device.

**What Happens:**
```
User scans QR → BLE connects
↓
App requests: Account Verification
↓
Account check: UNKNOWN / MISMATCH
↓
Device sends: "Access Denied" (explicit error)
↓
BLE connection stays OPEN (does NOT disconnect)
↓
App shows: "You are not the registered owner of this Distiller."
↓
User can switch accounts or contact support
```

**Result:** Security is enforced based on account ownership. Unregistered users cannot provision WiFi.
