...
The onboarding flow handles device discovery via Bluetooth, WiFi configuration, and security setup.
...
2. **Show Complexity When Needed**: Manual entry appears only when automatic fails
3. **Respect Time**: Auto-retry shows progress, not just waiting
4. **Clear Status**: Processing states explain what's happening
5. **Security Required**: PIN protects device; forgot PIN requires factory reset
