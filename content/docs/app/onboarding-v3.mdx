...
| **Subtitle** | "You're all set. Let's get started." — clean, minimal completion message. |
...
| Scenario | Auth | BLE Pairing | WiFi Setup | Claude Code | Entry Point |
| :--- | :--- | :--- | :--- | :--- | :--- |
...
<Callout type="info">
  **Skip is always available.** Users can skip device pairing entirely during first launch (lands in empty state, add device later from Settings). Claude Code setup is skippable. We never block the user.
</Callout>
...
flowchart TD
    ...
    ServiceSetup[Claude Code Setup] --> ServiceChoice{User action}
    ServiceChoice -->|Setup| ClaudeAuth[OAuth flow]
    ServiceChoice -->|I'll do this later| SetupComplete

    ClaudeAuth --> SetupComplete[Setup Complete]

    SetupComplete --> End([Main App])
...
After WiFi is connected (or confirmed from migration), the user configures Claude Code. This is a single-page setup — not a service chooser.

<Mermaid chart={`
flowchart TD
    WiFiDone([WiFi Connected / Confirmed]) --> ClaudeScreen[Claude Code Setup]

    ClaudeScreen --> Setup[User taps Setup]
    ClaudeScreen --> Skip[I'll do this later]

    Setup --> ClaudeAuth[User connects Anthropic account\nvia OAuth]
    ClaudeAuth --> ClaudeDone[Claude Code ready]

    ClaudeDone --> Complete[Setup Complete]
    Skip --> Complete

    style WiFiDone fill:#e1f5e1
    style ClaudeScreen fill:#4a9eff,color:#fff
    style Setup fill:#dbeafe
...
<Callout type="info">
  Claude Code can always be configured later from **Settings** in the main app. Skipping during onboarding is zero commitment.
</Callout>
...
| :--- | :--- | :--- | :--- |
| **Visual** | Distiller mascot | Distiller mascot | Distiller mascot |
| **Headline** | "[Device Name] is ready!" | "Your Distiller is updated!" | "[Device Name] is ready!" |
| **Subtitle** | "You're all set. Let's get started." | "You're all set. Let's get started." | "You're all set. Let's get started." |
| **Button** | "Go to [Device Name]" | "Go to [Device Name]" | "Done" (dismisses sheet) |
...
sequenceDiagram
    ...
    App->>User: Claude Code Setup screen
    Note over User,App: Setup / I'll do this later
...
- Open-Claw can be re-installed as an optional service later from Settings.
...
| **Connection lost during WiFi setup** | Alert: "Connection Lost" — "The connection to your Distiller was lost. You'll need to scan the QR code again." | "Start Over" → restart pairing |
...
| **Connection timeout (30s)** | Alert: "Connection Timed Out" — "Your Distiller couldn't connect to this network." | "Try Again" (resend) or "Different Network" |
