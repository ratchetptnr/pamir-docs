...
<Callout title="This supersedes previous onboarding docs">
  This document is the single source of truth for onboarding in Distiller OS v3.0+. It replaces [Onboarding](/docs/app/onboarding) (original spec) and [Changes to Onboarding](/docs/app/onboarding-changes) (architecture additions). Those docs remain for historical reference.
</Callout>
...
flowchart TD
    ...
    WiFiCheck -->|Yes - Migration| WiFiConfirm[Confirm existing WiFi\nor setup new]
    WiFiCheck -->|No - Fresh| WiFiSetup

    WiFiConfirm -->|Keep existing| ServiceSetup
    WiFiConfirm -->|Setup new WiFi| WiFiSetup
...
After BLE connects, the app reads the device state. If the device already has WiFi credentials (v2.1.1 migration), the user can skip this phase entirely.

<Mermaid chart={`
flowchart TD
    BLEConnected([BLE Connected]) --> HasWiFi{Device has\nWiFi credentials?}

    HasWiFi -->|Yes| Confirm[Show current network name\n+ confirm or change]
    HasWiFi -->|No| NetworkList

    Confirm -->|Keep| ServiceSetup([Service Setup])
    Confirm -->|Change| NetworkList
...
sequenceDiagram
    ...
    App->>User: "Device has WiFi configured.<br/>Keep current network or set up new?"

    alt Keep existing WiFi
        User->>App: Keep
        App->>Device: Verify WiFi connectivity
...
- After BLE connects, the app reads device state and discovers existing WiFi — offers to keep or change.
...
| Error | What user sees | Recovery |
| :--- | :--- | :--- |
| **Existing WiFi no longer works** | WiFi verification fails after "Keep" | Falls back to standard WiFi setup flow |
